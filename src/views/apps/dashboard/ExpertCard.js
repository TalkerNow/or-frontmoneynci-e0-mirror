...
class ExpertCard extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    await axios
      .get(global.config.server_url + "/getMembersPrestation", Config)
      .then((response) => {
        ...
        for (let j = 0; j < response.data.length; j += 1) {
          ...
          tmp_presta[j]["total creer Balance Clients"] =
            arr["creer Balance Clients"];
          tmp_presta[j]["total creer Termine"] = arr["creer Termine"];
          tmp_presta[j]["total CA En attente"] = arr["CA En attente"];
          tmp_presta[j]["total CA creer En attente"] =
            arr["CA creer En attente"];
          ...
          tmp_presta[j]["total CA creer Balance Clients"] =
            arr["CA creer Balance Clients"];
        }
  ...
  getTotalData(newYear) {
    axios
      .get(
        global.config.server_url + "/getMembersPrestation?year=" + newYear,
        Config
      )
      .then((response) => {
        ...
        for (let i = 0; i < response.data.length; i += 1) {
          ...
          tmp_presta[i]["total creer Balance Clients"] =
            t["creer Balance Clients"];
          ...
          tmp_presta[i]["total CA creer Balance Clients"] =
            t["CA creer Balance Clients"];
        }
        this.setState({ prestation: tmp_presta });
      });
  }
  ...
  // ----- UI handlers -----
  setDataExecutants = () =>
    this.setState({ creator: true, columnDefs: this.getCreatorsCols() });
  setDataCreators = () =>
    this.setState({ creator: false, columnDefs: this.getExecutantsCols() });
  onBtExport = () => {
        ...
        <CardHeader>
          ...
          <Nav tabs className="px-2" style={{ gap: 8, flexWrap: "wrap" }}>
            <div>
              {this.state.creator ? (
                <Button
                  className="mr-1 mb-2 mr-3"
                  outline
                  color="primary"
                  onClick={this.setDataCreators}
                >
                  Exécutants
                </Button>
              ) : (
                <Button
                  ...
                  outline
                  color="primary"
                  onClick={this.setDataExecutants}
                >
                  Créateurs
                </Button>
              )}
