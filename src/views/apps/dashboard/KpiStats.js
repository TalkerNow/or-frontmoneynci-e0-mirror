...
export default function KpiStats() {
  ...
  const fetchStatsWeek = useCallback(
    async (isMountedRef) => {
      try {
        ...
        for (const k of all) {
          ...
          if (!byDay[dayKey])
            byDay[dayKey] = { date: dayKey, entrants: 0, sortants: 0 };
        ...
        const ordered = Object.values(byDay).sort((a, b) =>
          a.date > b.date ? 1 : -1
        );
        if (isMountedRef.current) {
          ...
          <CardBody>
            <div className="d-flex justify-content-between align-items-center mb-2">
              <h5 style={{ fontWeight: 600 }}>
                Statistiques Activité (sem. {week})
              </h5>
