...
import { changeFilter, searchTask } from "../../../../../redux/actions/client-todo/index"

const ClientTaskNavbar = (props) => {
    const [searchTerm, setSearchTerm] = useState("")
    const { id, embedded, routeParam } = props

    useEffect(() => {
        // Sync local state if needed
    }, [])

    // Mode embedded : pas de navbar, le bouton est dans TaskList
    if (embedded) {
        return null
    }

    const handleFilterChange = (filter) => {
        props.changeFilter(id, filter, !!embedded);
    }
    ...
    const activeFilter = routeParam && routeParam.filter ? routeParam.filter : "all"
        ...
        <div className="task-navbar d-flex flex-wrap justify-content-between align-items-center bg-white px-2 py-1 mb-0 w-100 border-bottom">
            <div className="d-flex align-items-center flex-wrap">
                ...
                <UncontrolledDropdown className="mr-2">
                    <DropdownToggle color={currentIsLabel ? "primary" : "light"} outline={!currentIsLabel} caret size="sm">
...
export default connect(mapStateToProps, { changeFilter, searchTask })(ClientTaskNavbar)