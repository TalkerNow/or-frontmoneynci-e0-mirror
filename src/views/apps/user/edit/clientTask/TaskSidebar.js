...
class TaskSidebar extends React.Component {
  ...
  async componentDidUpdate(prevProps, prevState) {
    ...
    } else if (prevProps.addTaskState !== this.props.addTaskState && this.props.addTaskState) {
      // When modal opens for new task, set default receiver from props
      if (!this.props.taskToUpdate && this.props.receivers && this.props.receivers.length > 0) {
        this.setState({
          newTask: {
            ...this.state.newTask,
            receiver: this.props.receivers[0].value
          }
        })
      }
    }
                          ...
                          ? {
                            ...
                            label: this.props.receivers.find(
                              (obj) =>
                                obj.value ==
                                taskReceiver,
                            )?.label || this.props.taskToUpdate?.task_customer?.name,
                          }
                          : this.props.receivers.find(r => r.value === newTask.receiver)
                      }
                      name="client"
                      options={this.props.receivers}
                      placeholder="Sélectionner un client..."
                      isDisabled={true} // Lock selection to current client as per requirement
                      onChange={(option) => {
                        if (isEditing) {
